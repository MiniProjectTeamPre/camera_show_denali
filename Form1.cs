...
namespace camera_show_denali {
    public partial class Form1 : Form {
            ...
            if (flag_set_port == "set port") {
                ...
                while (flag_while) {
                    ...
                    try {
                        ...
                        else capture = new VideoCapture(Convert.ToInt32(File.ReadAllText("../../config/test_head_" + head + "_port.txt")), captureApi);
                        ...
                        else if (address == Address.saturation)
                        {
                            int saturationCsv = Convert.ToInt32(File.ReadAllText("../../config/test_head_" + head + "_saturation_" + steptest + ".txt"));
                            if (!CheckAddressCamera(saturationCsv))
                            {
                                ScanAddressCamera(saturationCsv);
                            }
                        }
                        else
                        {
                            int gainCsv = Convert.ToInt32(File.ReadAllText("../../config/test_head_" + head + "_gain_" + steptest + ".txt"));
                            if (!CheckAddressCamera(gainCsv))
                            {
                                ScanAddressCamera(gainCsv);
                            }
                        }
        ...
        private void setup() {
            ...
            string mjh = "";
            try { if (ctms_led_no.Text == "2") mjh = "_2"; } catch { }
            try { rect.X = Convert.ToInt32(File.ReadAllText("../../config/test_head_" + head + "_rect_x_" + steptest + mjh + ".txt")); } catch { }
            try { rect.Y = Convert.ToInt32(File.ReadAllText("../../config/test_head_" + head + "_rect_y_" + steptest + mjh + ".txt")); } catch { }
            try { rect.Width = Convert.ToInt32(File.ReadAllText("../../config/test_head_" + head + "_rect_width_" + steptest + mjh + ".txt")); } catch { }
            try { rect.Height = Convert.ToInt32(File.ReadAllText("../../config/test_head_" + head + "_rect_height_" + steptest + mjh + ".txt")); } catch { }
            try
            { ctms_setGammaAddressCsv.Text = File.ReadAllText("../../config/test_head_" + head + "_gamma_" + steptest + ".txt"); } catch { }
            try
            { ctms_setGainAddressCsv.Text = File.ReadAllText("../../config/test_head_" + head + "_gain_" + steptest + ".txt"); } catch { }
            try
            { ctms_setSaturationAddressCsv.Text = File.ReadAllText("../../config/test_head_" + head + "_saturation_" + steptest + ".txt"); } catch { }
            try
            {
                if (flag_led_test == 1 || flag_led_test == 4) {//แดง 0 60 0 255 150 255
                    hsv_low = new Hsv(0, 0, 150);
                    hsv_high = new Hsv(60, 255, 255);
                }
                if (flag_led_test == 2 || flag_led_test == 5) {//เขียว 70 100 100 255 0 50
                    bgr_low = new Bgr(0, 100, 0);
                    bgr_high = new Bgr(100, 150, 50);
                }
                if (flag_led_test == 3 || flag_led_test == 6) {//น้ำเงิน 200 210 0 50 0 50
        ...
        private void fail_check_led() {
            if (debug == true) { timeout.Restart(); return; }
        ...
        private void check_led(object sender, EventArgs e) {
            ...
            int redpixels = 0;
            if (flag_led_test == 1 || flag_led_test == 4) {//แดง
                ...
                Graphics g = Graphics.FromImage(img.Bitmap);
                if (flag_led_test == 2 || flag_led_test == 5) g.DrawRectangle(Pens.Lime, rect);
                else g.DrawRectangle(Pens.Blue, rect);
            ...
            if (mask == true) {
                if (flag_set_camera == false && debug == false) {
                    if (flag_led_test != 7) {
                        ...
                        if (flag_led_test == 4) ctms_led_no.Text = "2";
                        setup();
                        timeout.Restart();
                        return;
                    }
        ...
        private void ReadFileAddress() {
            ...
            {
                ctms_setSaturationAddress.Checked = true;
            }
            else
            {
                ctms_setGainAddress_.Checked = true;
            }
        }
        /// <summary>
        ...
        private void B_focus_Click(object sender, EventArgs e) {
            ...
            try { h_focus.Value = (int)capture.GetCaptureProperty(CapProp.Focus); } catch (Exception) { }
            s_focus.Text = h_focus.Value.ToString();
        }
        ...
        private void pictureBox1_MouseUp(object sender, MouseEventArgs e) {
            ...
            string mkmj = "";
            if (ctms_led_no.Text == "2") mkmj = "_2";
            File.WriteAllText("../../config/test_head_" + head + "_rect_x_" + steptest + mkmj + ".txt", rect.X.ToString());
            File.WriteAllText("../../config/test_head_" + head + "_rect_y_" + steptest + mkmj + ".txt", rect.Y.ToString());
            File.WriteAllText("../../config/test_head_" + head + "_rect_width_" + steptest + mkmj + ".txt", rect.Width.ToString());
            File.WriteAllText("../../config/test_head_" + head + "_rect_height_" + steptest + mkmj + ".txt", rect.Height.ToString());
        }
        ...
        private void ctms_led_no_Click(object sender, EventArgs e) {
            ...
            while (true) {
                ...
                try {
                    asd = Convert.ToInt32(input);
                    if (asd > 2 || asd < 1) throw new Exception();
                } catch {
                    MessageBox.Show("not format");
                    continue;
                }
        ...
        private void ctms_flag_test_Click(object sender, EventArgs e) {
            ...
            while (true) {
                ...
                try {
                    asd = Convert.ToInt32(input);
                    if (asd > 6 || asd < 1) throw new Exception();
                } catch {
                    MessageBox.Show("not format");
                    continue;
                }
        ...
        private void ctms_setSaturationAddress_Click(object sender, EventArgs e) {
            if (!CheckPasswordSetAddress())
            {
                return;
            }
            ctms_setGainAddress.Checked = false;
            ctms_setGammaAddress.Checked = false;
            ctms_setSaturationAddress.Checked = true;
            if (steptest_camera_read2d_flag)
            {
                File.WriteAllText("../../config/test_head_" + head + "_address_read2d.txt", Address.saturation);
            }
            else
            {
                File.WriteAllText("../../config/test_head_" + head + "_address.txt", Address.saturation);
            }
        }

        private void ctms_setGainAddress_Click(object sender, EventArgs e) {
            if (!CheckPasswordSetAddress())
            {
                return;
            }
            ctms_setGainAddress.Checked = true;
            ctms_setGammaAddress.Checked = false;
            ctms_setSaturationAddress.Checked = false;
            if (steptest_camera_read2d_flag)
            {
                File.WriteAllText("../../config/test_head_" + head + "_address_read2d.txt", Address.gain);
            }
            else
            {
                File.WriteAllText("../../config/test_head_" + head + "_address.txt", Address.gain);
            }
        }
        ...
        private void ctms_setSaturationAddressCsv_Click(object sender, EventArgs e) {
            string address = String.Empty;
            while (true)
            {
                string input = Microsoft.VisualBasic.Interaction.InputBox("Set saturation address in csv", "Saturation Address",
                    ctms_setSaturationAddressCsv.Text, 500, 300);
                if (input == "")
                    return;
                if (!CheckPasswordSetAddress())
                {
            ...
            ctms_setSaturationAddressCsv.Text = address;
            File.WriteAllText("../../config/test_head_" + head + "_saturation_" + steptest + ".txt", address.ToString());
        }

        private void ctms_setGainAddressCsv_Click(object sender, EventArgs e) {
            string address = String.Empty;
            while (true)
            {
                string input = Microsoft.VisualBasic.Interaction.InputBox("Set gain address in csv", "Gain Address",
                    ctms_setGainAddressCsv.Text, 500, 300);
                if (input == "")
                    return;
                if (!CheckPasswordSetAddress())
                {
            ...
            ctms_setGainAddressCsv.Text = address;
            File.WriteAllText("../../config/test_head_" + head + "_gain_" + steptest + ".txt", address.ToString());
        }
